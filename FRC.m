function varargout = FRC(varargin)
% FRC MATLAB code for FRC.fig
%      FRC, by itself, creates a new FRC or raises the existing
%      singleton*.
%
%      H = FRC returns the handle to a new FRC or the handle to
%      the existing singleton*.
%
%      FRC('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in FRC.M with the given input arguments.
%
%      FRC('Property','Value',...) creates a new FRC or raises
%      the existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before FRC_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      pushbuttonstop.  All inputs are passed to FRC_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help FRC

% Last Modified by GUIDE v2.5 17-Apr-2019 15:19:18

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FRC_OpeningFcn, ...
                   'gui_OutputFcn',  @FRC_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before FRC is made visible.
function FRC_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FRC (see VARARGIN)

% Choose default command line output for FRC
...
% UIWAIT makes FRC wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = FRC_OutputFcn(hObject, eventdata, handles)
...
if filterindex > 0
   ...
   FiberData_raw=Data1; % marker and fiber have same channel 
end
...
if filterindex > 0
   ...
   StimulusData2=Data2(:,1);
   PressureData=Data2(:,2);
   %remove baseline (first 30s)
   PressureData=PressureData-mean(PressureData(1:30*Fs2));
...
des = drawRect(movdata,pt,wSize,2);
%figure
axes(handles.axes1);
imshow(des);
%hold off;
...
if isnan(MovieMarker); 
    if exist('handles.RedX')
       set(handles.text13,'String','please push "Draw Light" button first '); 
        ...
        frames_light=FrameRate*1*60 ; % first 1 min
        for k = 1 : frames_light   
        ...
        threshold3=max(rdata_corrected)*0.8;
        % extract marker frame
...
if ~isnan(MarkerManual2)
    ...
    set(gca,'ylim',[min(handles.PressureData) max(y_need)])
    set(gca,'xlim',[0 max(length(handles.FiberData)/handles.Fs1,length(handles.PressureData)/handles.Fs2)])
    xlabel('Time (s)'); 
    ylabel('H2O (cm)');
    title('Pressure Data','color','r');    
    ...
    plot([x1 x1],[min(handles.PressureData) y1],'g-')
    hold off
end
